...
Safety rails: resume watchers deduplicate within a 2-minute window (plus an already-queued guard out to `AUTO_CONTINUE_MAX_WAIT` so long sleeps never stack a second watcher), a global ceiling of 20 detached resumes per day, per-session counters reset on a healthy turn end (which also acks the delivered `.draining` head; overload re-prompts never ack), the watcher acks `.draining` only when its resume exits 0 (else it stays for restore), queued prompts reach the resume command via the `$AC_PROMPT_FILE` temp file (never interpolated into the command string), the watcher never sends into a dead quota while a probe is configured (it re-arms on the backoff ladder instead, and probe failure counts as still-limited), and `~/.auto-continue/` is created `0700`.
