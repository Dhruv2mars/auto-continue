import { mkdir, readFile, writeFile, readdir, stat, rename, appendFile, unlink } from "node:fs/promises";
...
export async function loadState(sessionId) {
  try {
    ...
    return { continues: parsed.continues ?? 0, updatedAt: parsed.updatedAt ?? 0 };
  } catch {
